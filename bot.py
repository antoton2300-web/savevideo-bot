...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if not files:
                files = glob.glob(f'downloads/{user_id}/*.mov')
            if not files:
                files = glob.glob(f'downloads/{user_id}/*.webm')
            if not files:
        ...
        elif "tiktok" in error_text.lower() and ("block" in error_text.lower() or "403" in error_text):
            await msg.edit_text("❌ <b>TikTok временно заблокировал запрос</b>\n\nПопробуйте через 5-10 минут или используйте Instagram/YouTube", parse_mode='HTML')
        else:
...
async def show_commands_menu(query):
    ...
    commands_text = (
        ...
        "🔹 <b>ПРОБЛЕМЫ С TIKTOK:</b>\n"
        "• TikTok часто блокирует ботов\n"
        "• Если не работает - подождите 10-15 минут\n"
        "• Или попробуйте Instagram/YouTube\n\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "🔹 <b>ПРОБЛЕМЫ С TIKTOK:</b>\n"
        "• TikTok часто блокирует автоматические скачивания\n"
        "• Если не работает - подождите 10-15 минут\n"
        "• Попробуйте Instagram или YouTube\n"
